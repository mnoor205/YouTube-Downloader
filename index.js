...
async function mergeAudioAndVideo(videoTitle, audioPath, videoPath, mergeBar) {
  const sanitizedTitle = videoTitle?.replace(/[^a-z0-9]/gi, '_') || 'output';
  const outputPath = `${sanitizedTitle}.mp4`;
...
async function main() {
  ...
  try {
    ...
    progressBars.stop();
    console.log(`Download complete! Output saved as "${videoTitle.replace(/[^a-z0-9]/gi, '_')}.mp4"`);
    console.timeEnd()
  } catch (error) {
